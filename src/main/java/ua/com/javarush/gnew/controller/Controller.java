...
public class Controller implements Initializable {
    ...
    void choeseButtonCliced(ActionEvent event) {
        ...
        if (selectedFile != null) {
            ...
            if (dropdown.getValue().equals("Encrypt") || dropdown.getValue().equals("Decrypt")) {
    ...
    void keyInputSelected(KeyEvent event) {
        // Разблокировка кнопки DoCypher, если inputKey в фокусе и нажата клавиша
    ...
    void doCypher(ActionEvent event) {
        ...
        if (!actualPathEmpty && !dropdownEmpty) {
            ...
            // Блокируем элементы после выполнения шифрования/дешифрования
            if (!dropdown.getValue().equals("Bruteforce")) {
                inputKey.setDisable(true);
            }
            doCypherButton.setDisable(true);
            openNewFileDir.setDisable(false);

            // Очистка поля inputKey
            inputKey.clear();
        } else {
    ...
    void methodChoesed(ActionEvent event) {
        choesePathButton.setDisable(false); // Разблокировка кнопки ChoesePath при выборе метода
